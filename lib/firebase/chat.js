...
export const sendMessage = async (
  ...
  if (sender === "user") {
    ...
    updatePayload.userUnreadCount = increment(1);
  }
...
// Subscribe to User Unread Count (For Navbar)
export const subscribeToUnreadChatCount = (userId, callback) => {
  if (!userId) return () => {};
  const chatRef = doc(db, "chats", userId);
  return onSnapshot(chatRef, (snapshot) => {
    if (snapshot.exists()) {
      callback(snapshot.data().userUnreadCount || 0);
    } else {
      callback(0);
    }
  });
...
export const markUserChatRead = async (userId) => {
  if (!userId) return;
  const chatRef = doc(db, "chats", userId);
  await updateDoc(chatRef, {
    userUnreadCount: 0,
