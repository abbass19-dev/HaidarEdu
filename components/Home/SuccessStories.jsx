...
const TestimonialCard = ({ item }) => (
    <div className={`glass ${item.special ? 'gradient-border-card' : ''}`} style={{
        padding: '32px',
            ...
            <div>
                <div style={{ display: 'flex', gap: '4px', marginBottom: '16px' }}>
                    {[...Array(item.rating)].map((_, i) => (
                        <Star key={i} size={14} fill="var(--primary-lime)" color="var(--primary-lime)" />
                    ))}
                </div>
                <p style={{ fontSize: '1rem', lineHeight: '1.6', color: 'var(--text-main)', marginBottom: '24px' }}>"{item.text}"</p>
            </div>

            <div className="testimonial-meta" style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: 'auto' }}>
        ...
        <section style={{ padding: '100px 0', overflow: 'hidden' }}>
            <div className="container">
                <div style={{
                    ...
                    <div style={{ textAlign: 'left' }}>
                        ...
                        <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', maxWidth: '500px' }}>
                            Join thousands of traders who achieved financial freedom with HaidarEdu.
                        </p>
                    </div>
                ...
                <div className="marquee-container">
                    {/* Column 1: Upwards */}
                    <div className="marquee-column up slow">
                        {tripled.map((item, i) => <TestimonialCard key={i} item={item} />)}
                    </div>

                    {/* Column 2: Downwards */}
                    <div className="marquee-column down fast">
                        {tripled.slice().reverse().map((item, i) => <TestimonialCard key={i} item={item} />)}
                    </div>

                    {/* Column 3: Upwards */}
                    <div className="marquee-column up">
                        {[...tripled].reverse().map((item, i) => <TestimonialCard key={i} item={item} />)}
                    </div>
                </div>
            </div>
        </section>
